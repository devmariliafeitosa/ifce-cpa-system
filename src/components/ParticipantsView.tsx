...
import { BackendCampus, listCampuses } from '../services/campuses.service';

// ---------------------------------------------------------------
// Tipos locais (UI)
// ---------------------------------------------------------------
type ParticipantSegment = 'discente' | 'docente' | 'tae';
...
// ---------------------------------------------------------------
const roleToSegment = (roles: BackendRole[]): ParticipantSegment => {
  if (roles.includes('aluno')) return 'discente';
  if (roles.includes('docente')) return 'docente';
  return 'tae';
};

const segmentToRole = (segment: ParticipantSegment): BackendRole => {
  switch (segment) {
    case 'discente':
      return 'aluno';
    case 'docente':
...
const extractMatricula = (
  segment: ParticipantSegment,
  dados?: DadosRoles,
): string => {
  if (!dados) return '';
  if (segment === 'discente' && dados.aluno) return dados.aluno.matricula;
  if (segment === 'docente' && dados.docente)
    return String(dados.docente.siape);
  if (segment === 'tae' && dados.servidor)
    return String(dados.servidor.siape);
  return '';
};

const mapBackendToParticipant = (
  user: BackendUser,
  campusesMap: Record<string, string>,
): Participant => {
  const segment = roleToSegment(user.roles);
  const studentLevel =
    segment === 'discente' && user.dadosRoles?.aluno
      ? (user.dadosRoles.aluno.curso as StudentLevelType)
      : undefined;
  ...
  return {
    ...
    segment,
    studentLevel,
    matricula: extractMatricula(segment, user.dadosRoles),
    campusId: user.campusId,
    campusName: campusesMap[user.campusId] || 'Campus não identificado',
...
export const ParticipantsView: React.FC = () => {
  ...
  const [segmentFilter, setSegmentFilter] = useState<
    'todos' | ParticipantSegment
  >('todos');
  const [levelFilter, setLevelFilter] = useState<'todos' | StudentLevelType>(
    'todos',
  );
  ...
  const [importSegment, setImportSegment] =
    useState<ParticipantSegment>('discente');
  const [importLevel, setImportLevel] =
    useState<StudentLevelType>('Graduação');
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [users, camps] = await Promise.all([
        listUsers(),
        listCampuses(false),
      ]);
      setCampuses(camps);
      const cMap: Record<string, string> = {};
      camps.forEach((c) => (cMap[c.id] = c.nome));
      setParticipants(users.map((u) => mapBackendToParticipant(u, cMap)));

      if (camps.length > 0 && !formData.campusId) {
        setFormData((prev) => ({ ...prev, campusId: camps[0].id }));
      }
    } catch (err: any) {
      console.error(err);
      setError(
        err?.message || 'Não foi possível carregar os participantes.',
      );
    } finally {
      setLoading(false);
    }
  ...
  const validateMatriculaInput = (
    ...
    if (!trimmed) {
      return {
        isValid: false,
        errorMessage:
          'A matrícula é obrigatória e atua como chave única do participante.',
        formattedMatricula: '',
      };
    }
    ...
    return { isValid: true, formattedMatricula: trimmed };
  };
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      const role = segmentToRole(formData.segment);

      const dadosPorRole: DadosRoles = {};
      if (formData.segment === 'discente') {
        dadosPorRole.aluno = {
          curso: formData.studentLevel,
          matricula: matriculaCheck.formattedMatricula,
          semestre: '1',
        };
      } else if (formData.segment === 'docente') {
        dadosPorRole.docente = {
          siape: Number(matriculaCheck.formattedMatricula.replace(/\D/g, '')),
        };
      } else {
        dadosPorRole.servidor = {
          siape: Number(matriculaCheck.formattedMatricula.replace(/\D/g, '')),
        };
      }

      if (editingParticipant) {
        await updateUser(editingParticipant.id, {
        ...
        showToast(
          `Participante "${formData.name}" atualizado com sucesso!`,
          'success',
        );
      } else {
        await createUser({
        ...
        showToast(
          `Participante "${formData.name}" cadastrado com sucesso!`,
          'success',
        );
      }
      ...
      console.error(err);
      showToast(
        err?.message || 'Erro ao salvar participante.',
        'danger',
      );
    } finally {
  ...
  const handleToggleStatus = async (p: Participant) => {
    ...
    try {
      if (newStatus === 'Inativo') {
        ...
        await updateUser(p.id, { ativo: true });
      }
      showToast(
      ...
      console.error(err);
      showToast(
        err?.message || 'Erro ao alterar status do participante.',
        'danger',
      );
    }
  };
  ...
  const handleDeleteParticipant = async (id: string) => {
    ...
    try {
      ...
      if (target) {
        showToast(
          `Participante "${target.name}" desativado com sucesso.`,
          'info',
        );
      }
      await loadData();
      ...
      console.error(err);
      showToast(
        err?.message || 'Erro ao desativar participante.',
        'danger',
      );
    } finally {
      setActionLoading(false);
  ...
  // -------------------------------------------------------------
  // Importação em massa (via createUser em loop)
  // -------------------------------------------------------------
  const handleBulkImport = async (e: React.FormEvent) => {
    ...
    if (campuses.length === 0) {
      showToast('Nenhum campus disponível para importação.', 'danger');
      return;
    }
    ...
    for (let idx = 0; idx < lines.length; idx++) {
      ...
      if (!email.includes('@')) {
        ...
        email = `${cleanName}@${
          importSegment === 'discente' ? 'aluno.ifce.edu.br' : 'ifce.edu.br'
        }`;
      }
      ...
      const role = segmentToRole(importSegment);
      const fakeMatricula =
      ...
      const dadosPorRole: DadosRoles = {};
      if (importSegment === 'discente') {
        dadosPorRole.aluno = {
          curso: importLevel,
          matricula: fakeMatricula,
          semestre: '1',
        };
      } else if (importSegment === 'docente') {
        dadosPorRole.docente = { siape: Number(fakeMatricula) };
      } else {
        dadosPorRole.servidor = { siape: Number(fakeMatricula) };
      }
    ...
    if (successCount > 0 && errorCount === 0) {
      showToast(`${successCount} participantes importados com sucesso!`, 'success');
      ...
      showToast('Nenhum participante pôde ser importado.', 'danger');
    }
  ...
  const filteredParticipants = useMemo(() => {
    return participants.filter((p) => {
      ...
      const matchSegment =
        segmentFilter === 'todos' || p.segment === segmentFilter;
      ...
      const matchCampus =
        campusFilter === 'todos' || p.campusId === campusFilter;

      return (
        matchQuery && matchSegment && matchLevel && matchStatus && matchCampus
      );
    });
  }, [
    participants,
    ...
    <div className="w-full max-w-[96%] 2xl:max-w-[1440px] mx-auto px-2 sm:px-4 py-4 space-y-4 select-none animate-in fade-in duration-200">
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="p-3 rounded-lg text-xs font-semibold flex items-center justify-between shadow-2xs border bg-rose-50 border-rose-200 text-rose-800"
          >
            ...
            <button
              onClick={loadData}
              className="flex items-center gap-1 text-rose-700 hover:underline font-bold cursor-pointer"
            >
      ...
      >
        ...
        <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
          <button
            onClick={loadData}
            className="h-8 px-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-xs rounded-lg shadow-2xs transition-all flex items-center justify-center gap-1.5 cursor-pointer"
            title="Recarregar"
          >
        ...
        <div className="bg-white rounded-xl border border-slate-200/90 p-3.5 shadow-2xs hover:border-slate-300 transition-all flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium mt-1">
              <strong className="text-[#006837] font-bold">
                {stats.ativosCount}
              </strong>{' '}
              ativos (
              {stats.total > 0
                ? Math.round((stats.ativosCount / stats.total) * 100)
                : 0}
              %)
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200/90 p-8 shadow-2xs text-center space-y-3 max-w-md mx-auto my-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              Nenhum registro corresponde aos filtros ou termo de busca
              aplicados.
            </p>
          </div>
          <div className="pt-1">
              ...
              >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setFormData({ ...formData, segment: seg.id })
                        }
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          studentLevel: e.target.value as StudentLevelType,
                        })
                      }
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                  <button
                    ...
                    className="h-8 px-3 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer disabled:opacity-50"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    disabled={actionLoading}
                    className="h-8 px-3 bg-[#006837] hover:bg-[#00522b] text-white text-xs font-bold rounded-lg shadow-2xs transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                  >
                    {actionLoading && (
          ...
          <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  O participante será marcado como inativo e não poderá
                  acessar o sistema, mas seus dados permanecerão na base.
                </p>
              </div>
              <div className="pt-2 flex items-center justify-center gap-2">
                <button
                  ...
                  disabled={actionLoading}
                  className="h-8 px-3 text-xs font-bold text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
                  ...
                  disabled={actionLoading}
                  className="h-8 px-3 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg shadow-2xs cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                >
                  {actionLoading && (
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                  )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">
                      Cole uma lista de nomes e e-mails para cadastro
                      simultâneo.
                    </p>
                  </div>
                </div>
              ...
              <form onSubmit={handleBulkImport} className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-2.5">
                  <div className="space-y-1">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setImportSegment(
                          e.target.value as ParticipantSegment,
                        )
                      }
                  ...
                  {importSegment === 'discente' && (
                    <div className="space-y-1">
                      ...
                      <select
                        value={importLevel}
                        onChange={(e) =>
                          setImportLevel(e.target.value as StudentLevelType)
                        }
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-end gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={actionLoading}
                    className="h-8 px-3 bg-[#006837] hover:bg-[#00522b] text-white text-xs font-bold rounded-lg shadow-2xs cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                  >
                    {actionLoading && (
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    )}
