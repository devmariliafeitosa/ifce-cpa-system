...
const loginSchema = z.object({
  email: z
    ...
    .email("E-mail em formato inv├ílido.")
...
export const LoginScreen: React.FC<LoginScreenProps> = ({
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    if (foundCoordinator || isNewlyRegisteredUser) {
      ...
      if (
        foundCoordinator &&
        data.password !== foundCoordinator.password &&
        data.password !== "123456"
      ) {
        setAuthError("Senha incorreta para a conta de Coordena├º├úo.");
        return;
      }
      onLoginSuccess(data.email);
    } else {
      setAuthError(
        "Email ou senha inv├ílidos. Apenas e-mails de Coordena├º├úo cadastrados (ex: cpa.taua@ifce.edu.br) possuem permiss├úo.",
      );
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-500 font-normal">
                Fa├ºa login para acessar o painel da Coordena├º├úo da CPA.
              </p>
            </div>
        ...
        <div className="hidden md:col-span-5 lg:col-span-6 md:flex flex-col justify-between p-8 lg:p-12 bg-gradient-to-br from-[#0B7A3E] to-[#045C2D] text-white relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-3">
            ...
            <p className="text-sm text-emerald-100/90 leading-relaxed font-normal max-w-md">
              Plataforma respons├ível pela gest├úo das avalia├º├Áes
              institucionais do IFCE Campus Tau├í.
            </p>
          </div>
          ...
          <div className="relative z-10 text-xs text-emerald-200/80 font-medium border-t border-white/15 pt-4 flex items-center justify-between">
            <span>Avalia├º├úo Institucional Permanente</span>
            <span>Campus Tau├í</span>
          </div>
        </div>
      </div>
    </div>
