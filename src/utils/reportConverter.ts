import { SmartForm, SmartQuestion } from '../types';
import {
  ReportCampaignData,
  ReportQuestion,
...
export function convertSmartFormToReportCampaign(form: SmartForm): ReportCampaignData {
  const totalResponses = form.responsesCount?.total ?? 0;
  const questionsCount = form.questions?.length ?? 0;

  // Extract year & semester
  let year = '2026';
  const yearMatch = (form.title + ' ' + (form.periodo || '') + ' ' + (form.startDate || '')).match(/20\d{2}/);
  if (yearMatch) {
    year = yearMatch[0];
  }

  let semester = `${year}.2`;
  const semMatch = (form.title + ' ' + (form.periodo || '')).match(/\b20\d{2}\.[12]\b/);
  if (semMatch) {
    semester = semMatch[0];
  ...
  const isFinished = currentStatusStr.includes('encerrad') || currentStatusStr.includes('finalizad');
  const status: 'Finalizada' | 'Em andamento' = isFinished ? 'Finalizada' : 'Em andamento';

  // Response rate calculation
  const estimatedCampusUniverse = 1600;
  const responseRate = totalResponses > 0
    ? Math.min(99.9, Math.round((totalResponses / estimatedCampusUniverse) * 1000) / 10)
    : 0;
    ...
    targetSegments.forEach((seg) => {
      ...
      if (segmentAnswers === 0) {
        ...
        if (q.type === 'SCALE') {
          ...
          alternatives = q.options.map((opt) => ({ option: opt, count: 0, percentage: 0 }));
        } else {
          alternatives = [
        ...
        if (q.type === 'SCALE') {
          ...
          if (hashSeed === 1) {
            pOtimo = 78; pRegular = 14; pRuim = 5; pNa = 3;
          } else if (hashSeed === 2) {
            pOtimo = 48; pRegular = 32; pRuim = 15; pNa = 5;
          }
...
export function buildReportsFromSmartForms(forms: SmartForm[]): ReportCampaignData[] {
