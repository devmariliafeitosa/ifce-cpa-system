...
import { SmartForm, SmartQuestion, StudentLevel } from '../../../types';

/* Modal de "Preenchimento Inteligente" — simula o preenchimento do formulário pelo participante,
 * com filtragem de perguntas por segmento/nível e validação de campos obrigatórios.
 * Extraído de FormsManagerView.tsx. Todo o estado deste fluxo era usado exclusivamente aqui,
 * então virou estado interno do próprio componente (em vez de props avulsas). */

type Segment = 'alunos' | 'docentes' | 'taes';

interface RespondFormModalProps {
  form: SmartForm | null;
  onClose: () => void;
  onSubmitted: (formId: string, segment: Segment) => void;
  showNotification: (type: 'success' | 'error' | 'info', message: string) => void;
}

export const RespondFormModal: React.FC<RespondFormModalProps> = ({
  form,
  onClose,
  onSubmitted,
  showNotification,
}) => {
  const [participantSegment, setParticipantSegment] = useState<Segment | null>(null);
  ...
  // Reseta todo o estado interno sempre que um novo formulário é aberto para resposta.
  useEffect(() => {
    if (form) {
  ...
  const handleSubmitParticipantResponse = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      onSubmitted(form.id, participantSegment);
      setIsSubmittingResponse(false);
      setResponseSubmitted(true);
      showNotification(
        'success',
        `Resposta enviada com sucesso para o segmento ${
          participantSegment === 'alunos' ? 'Aluno' : participantSegment === 'docentes' ? 'Docente' : 'TAE'
        }!`
      );
    }, 600);
  };
          ...
          <div className="p-6 text-center space-y-4 my-auto animate-in zoom-in-95">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Sua resposta para o formulário "{form.title}" foi registrada com sucesso pela CPA do IFCE Campus Tauá.
              </p>
            </div>
            <button
              type="button"
