...
/* Métricas de resposta por público-alvo de um formulário. Extraído de FormsManagerView.tsx. */

interface FormMetricsDialogProps {
  form: SmartForm | null;
  onClose: () => void;
}

export const FormMetricsDialog: React.FC<FormMetricsDialogProps> = ({ form, onClose }) => {
  if (!form) return null;
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-3 text-center">
            <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
              ...
              <p className="text-base font-bold text-slate-900">{form.responsesCount.total}</p>
            </div>
            <div className="p-3 bg-indigo-50 rounded-xl border border-indigo-100">
              <p className="text-[10px] text-indigo-600 font-semibold uppercase">Alunos</p>
              <p className="text-base font-bold text-indigo-900">{form.responsesCount.alunos}</p>
            </div>
            <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100">
              <p className="text-[10px] text-emerald-700 font-semibold uppercase">Docentes</p>
              <p className="text-base font-bold text-emerald-900">{form.responsesCount.docentes}</p>
            </div>
            <div className="p-3 bg-amber-50 rounded-xl border border-amber-100">
              <p className="text-[10px] text-amber-700 font-semibold uppercase">TAEs</p>
              <p className="text-base font-bold text-amber-900">{form.responsesCount.taes}</p>
            </div>
          </div>
